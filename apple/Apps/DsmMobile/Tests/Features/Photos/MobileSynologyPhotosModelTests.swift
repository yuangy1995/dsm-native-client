...
@MainActor
final class MobileSynologyPhotosModelTests: XCTestCase {
    func test时间轴按日期定位且不依赖时间轴数量() async {
        let repository = MobileDateMobilePhotoServiceStub()
        let model = MobileSynologyPhotosModel(repository: repository, pageSize: 2)
        await model.refresh()
    ...
    func test向前分页迟到不能覆盖刷新结果且并发触发只请求一次() async {
        let repository = MobilePhotoServiceStub(pages: [[Self.photo], [], []], days: [
            .init(year: 2026, month: 9, day: 1, itemCount: 1),
            .init(year: 2013, month: 1, day: 1, itemCount: 1)
        ])
        let model = MobileSynologyPhotosModel(repository: repository, pageSize: 1)
        await model.refresh()
    ...
    func test筛选候选失败不污染照片列表的错误状态() async {
        let repository = MobilePhotoServiceStub(pages: [[Self.photo]])
        let model = MobileSynologyPhotosModel(repository: repository)
        await model.refresh()
    ...
    func test自动分页每次只读取下一页且重复结果停住() async {
        let repository = MobilePhotoServiceStub(pages: [[Self.photo], [Self.photo], []])
        let model = MobileSynologyPhotosModel(repository: repository, pageSize: 1)
    ...
    func test旧刷新迟到不能恢复已删除照片() async {
        let repository = MobilePhotoServiceStub(pages: [[]], holdsFirstPage: true)
        let model = MobileSynologyPhotosModel(repository: repository)
    ...
    func test空时间线不额外扫描或读取媒体() async {
        let repository = MobilePhotoServiceStub(pages: [], days: [])
        let model = MobileSynologyPhotosModel(repository: repository)
        await model.refresh()
        let count = await repository.pageRequestCount
    ...
    func test翻页沿用套件原始数量而不是去重后数量() async {
        let second = Self.photoWithID(8)
        let third = Self.photoWithID(9)
        let repository = MobilePhotoServiceStub(pages: [[Self.photo, second], [second, third], []])
        let model = MobileSynologyPhotosModel(repository: repository, pageSize: 2)
        await model.refresh()
        await model.loadMore()
        XCTAssertEqual(model.items.map(\.id.unitID), [7, 8, 9])
        await model.loadMore()
    ...
    func test新平台删除默认关闭且不能绕过确认() async {
        let repository = MobilePhotoServiceStub(pages: [[Self.photo]])
        let model = MobileSynologyPhotosModel(repository: repository)
        await model.refresh()
        XCTAssertFalse(model.canDeleteOriginals)
        model.requestDeletion(Self.photo)
        XCTAssertNil(model.deletionCandidate)
        XCTAssertNotNil(model.deletionError)
        model.confirmDeletion(Self.photo)
        XCTAssertFalse(model.isDeleting)
        XCTAssertEqual(model.items, [Self.photo])
    }

    func test账号退出清空可见记录与候选且迟到请求不得恢复() async {
        let repository = MobilePhotoServiceStub(pages: [], holdsFirstPage: true)
        let model = MobileSynologyPhotosModel(repository: repository)
        let request = Task { await model.refresh() }
        await repository.waitForHeldPage()
    ...
    private static func photoWithID(_ id: Int) -> SynologyPhoto {
        SynologyPhoto(id: .init(profileID: photo.id.profileID, space: .personal, unitID: id),
            filename: "fixture.jpg", sizeBytes: 128, takenAt: photo.takenAt, indexedAt: photo.indexedAt,
            folderID: 9, mediaType: "photo")
    }

    func test未加载失败可以重试() async {
        let repository = MobilePhotoServiceStub(pages: [[Self.photo]], failsFirstAccess: true)
        let model = MobileSynologyPhotosModel(repository: repository)
        await model.loadIfNeeded()
        XCTAssertNotNil(model.errorMessage)
...
private actor MobileDateMobilePhotoServiceStub: SynologyPhotosServing {
    struct Request {
        let start: Int
        let end: Int
        let offset: Int
...
private actor MobilePhotoServiceStub: SynologyPhotosServing {

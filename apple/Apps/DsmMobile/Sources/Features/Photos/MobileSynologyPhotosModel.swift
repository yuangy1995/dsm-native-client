...
/// 移动照片展示状态，来源为 macOS 同名模型的 2243e731 基线；网络/领域继续共享 DsmCore、DsmNetwork。
/// 触控展示、系统导出和有界缓存属于移动端；Mac App 源码不在本轮修改范围。
/// 刷新、离开和账号切换隔离迟到响应，不能重新显示上一会话的数据。
@MainActor
@Observable
final class MobileSynologyPhotosModel {
    ...
    private(set) var datedGroups: [(date: Date, photos: [SynologyPhoto])] = []
    ...
    @ObservationIgnored let thumbnailStore = MobilePhotoThumbnailStore(totalCostLimit: 24 * 1_024 * 1_024, concurrencyLimit: 4)
    @ObservationIgnored private let permitsDeletion: Bool
    private(set) var exportPresentation: MobileDocumentPresentation?
    @ObservationIgnored private var exportDirectory: URL?
    @ObservationIgnored private var exportGeneration: UInt64 = 0

    init(repository: (any SynologyPhotosServing)? = nil, pageSize: Int = 100, permitsDeletion: Bool = false) {
        self.repository = repository
        self.pageSize = min(500, max(1, pageSize))
        self.permitsDeletion = permitsDeletion
    }
    ...
    /// 只在分页结果变更时分组；缩略图、下载进度和玻璃动画不会重复扫描整张列表。
    private func rebuildDatedGroups() {
        ...
        datedGroups = grouped.keys.sorted(by: >).map { ($0, grouped[$0] ?? []) }
    }

    /// 手机上直接切换到较新月份，避免一次读取整月再插入导致内存和网络峰值。
    func openNewerMonth() async {
        guard let current = previousMonthID,
              let month = timelineMonths.last(where: { $0.id > current }) else { return }
        await jumpToMonth(month)
    }
    ...
    func thumbnail(_ photo: SynologyPhoto) async throws -> Data {
        let current = generation
        let repository = try service()
        let namespace = photo.id.profileID.uuidString
        let key = "\(namespace)|\(photo.id.space.rawValue)|\(photo.id.unitID)|\(photo.thumbnail?.unitID ?? 0)|\(photo.thumbnail?.revision ?? "")"
        let data = await thumbnailStore.data(for: key, namespace: namespace, priority: .visible) {
        ...
        guard let data else {
            throw AppError(category: .invalidResponse, isRetryable: true, safeUserMessage: L10n.string("photos.media.failed"))
        }
        return data
    }
    ...
    func setModuleEnabled(_ enabled: Bool) {
        ...
        if !enabled {
            ...
            completeExport()
            Task { await thumbnailStore.removeAll() }
        }
    }
    ...
    var previewIdentity: String { "\(previewGeneration):\(previewData?.count ?? 0)" }

    func canPreviewAdjacent(_ direction: Int) -> Bool {
        guard let id = previewPhoto?.id, let index = items.firstIndex(where: { $0.id == id }) else { return false }
        return items.indices.contains(index + direction)
    }

    func retryCurrentPage() async {
        if !hasLoaded || (items.isEmpty && collections.isEmpty && sharedEntries.isEmpty) { await refresh(); return }
        errorMessage = nil
        if hasMoreCollections { await loadMoreCollections() }
        else if hasMore { await loadMore() }
        else { await refresh() }
    ...
    func prepareExport(_ photo: SynologyPhoto, intent: MobileDocumentIntent) {
        guard isModuleEnabled, !isSaving, intent != .upload else { return }
        completeExport()
        let operation = exportGeneration
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("lanstash-photos-" + UUID().uuidString, isDirectory: true)
        let destination = directory.appendingPathComponent(MobileDocumentTransferController.safeLeafName(photo.filename))
        ...
        saveTask = Task { [weak self] in
            guard let self else { return }
            defer { if operation == self.exportGeneration { self.isSaving = false } }
            do {
                try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: false)
                try await self.service().downloadOriginal(photo, to: destination) { [weak self] done, total in
                    Task { @MainActor in
                        guard let self, operation == self.exportGeneration else { return }
                        self.saveProgress = total.flatMap { $0 > 0 ? min(1, Double(done) / Double($0)) : nil }
                    }
                }
                try Task.checkCancellation()
                guard self.isModuleEnabled, operation == self.exportGeneration else {
                    try? FileManager.default.removeItem(at: directory)
                    return
                }
                self.exportPresentation = MobileDocumentPresentation(taskID: UUID(), profileID: photo.id.profileID,
                    url: destination, intent: intent)
            } catch {
                try? FileManager.default.removeItem(at: directory)
                guard operation == self.exportGeneration else { return }
    ...
    func suspend() {
        closePreview()
        // 已交给系统分享的文件由分享完成回调清理，不能在扩展读取时提前删除。
        if exportPresentation == nil { completeExport() }
    }
    ...
    func requestDeletion(_ photo: SynologyPhoto) {
        guard permitsDeletion, isModuleEnabled, !isDeleting, !isCheckingDeletion, pendingDeletionPhoto == nil else { return }
        let current = generation
    ...
    func confirmDeletion(_ photo: SynologyPhoto) {
        guard permitsDeletion, isModuleEnabled, !isDeleting, deletionCandidate?.id == photo.id else { return }
        deletionCandidate = nil
        isDeleting = true
        generation += 1
        isLoadingPrevious = false

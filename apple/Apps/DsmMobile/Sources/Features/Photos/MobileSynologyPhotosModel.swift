...
/// 移动端只读图库，查询与双向分页对齐 Mac 2243e731；触控预览与系统导出由本端管理。
/// 不将 File Station 路径当作 Photos 身份，也不向界面暴露远端写操作。
@MainActor
@Observable
final class MobileSynologyPhotosModel {
    ...
    private(set) var items: [SynologyPhoto] = [] { didSet { cachedGroups = nil } }
    ...
    private(set) var selectedCategory: SynologyPhotoCategory? { didSet { cachedGroups = nil } }
    ...
    private(set) var previewData: Data? { didSet { previewRevision &+= 1 } }
    private(set) var previewRevision: UInt64 = 0
    ...
    @ObservationIgnored private var cachedGroups: [(date: Date, photos: [SynologyPhoto])]?
    ...
    func loadPreviousPage() async {
        guard isModuleEnabled, hasPrevious, !isLoading, !isLoadingPrevious,
    ...
    var datedGroups: [(date: Date, photos: [SynologyPhoto])] {
        ...
        let result = grouped.keys.sorted(by: >).map { ($0, grouped[$0] ?? []) }
        cachedGroups = result
        return result
    }
    ...
    func loadMore() async {
        guard isModuleEnabled, hasMore, !isLoading, !isLoadingMore else { return }
        let current = generation
    ...
    func thumbnail(for photo: SynologyPhoto) async throws -> Data {
        ...
        let key = "\(namespace)|\(photo.id.space.rawValue)|\(photo.id.unitID)|\(photo.thumbnail?.revision ?? "")"
        let data = await thumbnails.data(for: key, namespace: namespace, priority: .visible) {
    ...
    func resetSearchAndFilters() async {
        searchText = ""
        filter = SynologyPhotoFilter()
        await refresh()
    }
    ...
    /// 导出到独立临时目录，系统分享结束、离开页面或切换账户后清理；不写 NAS。
    func exportOriginal(_ photo: SynologyPhoto) {
        ...
        let component = (photo.filename as NSString).lastPathComponent
        let filename = component.isEmpty || component == "." || component == ".." ? "\(photo.id.unitID)" : component
        ...
        saveTask = Task { [weak self] in
            guard let self else { return }
            var readyForSharing = false
            defer {
                if !readyForSharing { try? FileManager.default.removeItem(at: directory) }
                if current == self.exportGeneration { self.isSaving = false }
            }
            do {
                ...
                guard current == self.exportGeneration, self.isModuleEnabled else { return }
                self.exportURL = destination
                readyForSharing = true
            } catch is CancellationError {
                // 用户离开或取消时不显示旧任务错误。
            } catch {
                guard current == self.exportGeneration else { return }

...
BUILD_NUMBER_OVERRIDE="${LANSTASH_BUILD_NUMBER:-}"
...
validate_xcode_source_membership
if [[ -n "$BUILD_NUMBER_OVERRIDE" \
    && ! "$BUILD_NUMBER_OVERRIDE" =~ ^[0-9]+([.][0-9]+){0,2}$ ]]; then
    fail "LANSTASH_BUILD_NUMBER 必须是数字，最多包含三段点分数字"
fi
...
if [[ -n "$BUILD_NUMBER_OVERRIDE" ]]; then
    XCODEBUILD_OVERRIDES+=("CURRENT_PROJECT_VERSION=$BUILD_NUMBER_OVERRIDE")
fi

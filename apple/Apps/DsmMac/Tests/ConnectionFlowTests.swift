...
final class ConnectionFlowTests: XCTestCase {
    ...
    func test待上传修改阻止主动退出及删除连接但不清除登录状态() async throws {
        ...
        XCTAssertTrue(model.statusIsError)
        XCTAssertTrue(workspaceBefore.statusIsError)
        XCTAssertEqual(workspaceBefore.statusMessage, L10n.string("desktopDrive.writeback.pending"))
        XCTAssertEqual(workspaceBefore.activeToast?.text, L10n.string("desktopDrive.writeback.pending"))

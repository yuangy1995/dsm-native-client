...
照片模块正在按用户授权执行 [Synology Photos 完整替换](../development/NATIVE_DSM_PHOTOS_DEVELOPMENT_PLAN_ZH.md)：五端目标不再使用 File Station 扫描照片库；macOS 已切换个人照片时间线、搜索、相册、根文件夹和媒体读取入口，并按用户授权跨 DSM／Photos 版本依接口能力开放个人空间单项删除供测试。共享空间和其他写操作仍关闭，其他端尚未迁移，仓库旧照片源码尚未全量清理。下方既有范围和验证记录不是本次完整替换完成证据。

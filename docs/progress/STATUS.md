...
| Windows 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建、单任务删除、当前活动速率摘要、BT 搜索与 ACT-01 首片均已完成当前源码和 Windows 云端构建闭环；真实设备/真实 NAS 待验收 | Files/位置/预览/传输/分享、PHOTO-01、证书信任、FILE-03/05/09、PHOTO-03A、Chat 受限纯文字及单附件发送和 Download 常用单任务流程已进入 WinUI。BTSearch 固定公开 v1，完成能力门、原生 ContentDialog、`Ctrl+B`、提供方/类别/标题筛选及七类排序与方向、200 字与控制字符门、内存隐私说明、取消/关闭、零提供方、空/筛选空/错误/内容状态、A→B→A 迟到隔离、`_allcat_` 展示去重和单结果创建；创建继续复用既有 typed 单链接链，未知结果同会话不重放。ACT-01 首片让 DownloadStationViewModel 把当前任务快照同步给 ForegroundTransferCoordinator，Activity 显示 App 传输 / NAS 下载任务来源和暂停状态，取消按钮仅对 App running 任务可见。BTSearch 英中资源各 61 项，专项测试为 Repository 8 项、ViewModel 15 项和 source-contract 3 项，共 26 项；当前分支本地化、XML、契约与差异门通过，资源统计为 Apple 3526 / Android 1985 / Windows 1097。正式 ACT-01 提交 `2491212` 的 GitHub `Windows Build` run `31360092210` 通过 889/889 项 .NET xUnit，WinUI x64 与 ARM64 均构建通过；`Repository Check` run `31360092211` 通过。第 5 波 Chat 附件又在 GitHub `Windows Build` run `31384177338` 通过 921/921 项 .NET xUnit，并完成 WinUI x64 与 ARM64 构建；`Repository Check` run `31384179104` 通过。删除已下载文件/数据、批量、RSS、文件优先级、BT 协议高级设置、设置写、Activity 主动后台刷新和系统通知继续关闭；真实 NAS、Narrator、键盘、高对比和窗口缩放继续 `PENDING_USER_VALIDATION`。 |
...
3. 与 Chat 契约并行推进 **NAS-02/NAS-04 有界只读详情**：Apple 复用既有 typed 读取，Windows 先拆分区独立失败、分页、partial/truncated 契约；不接断开连接、套件生命周期、任务执行或设置写。

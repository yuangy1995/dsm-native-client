...
| iPhone/iPad 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建、单任务删除、当前活动速率摘要、BT 搜索与 ACT-01 首片均已完成当前源码和云端构建闭环；真机/真实 NAS 待验收 | Files/位置/预览/分享、新建/重命名、同 NAS 复制/移动、回收站、PHOTO-01/02/03A、Chat 只读、受限纯文字及单附件发送、Download 常用单任务流程和本地设置已形成移动主流程。BTSearch 固定公开 v1 的目录、启动、列表与清理；能力门、会话内输入、200 字/控制字符门、当前目录归属、搜索中冻结条件但允许关闭、条件变化取消与清旧结果、零提供方恢复态、独立清理、结果创建复用既有单链接安全链均已接入。ACT-01 首片将 Download Station 当前已加载任务快照同步为 Activity 的 NAS 来源，新增暂停态，NAS 项不显示取消或重试。BTSearch 英中资源各 48 项；本机通过共享聚焦 65/65、共享包全量 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone 模拟器 11/11；当前分支本地化为 Apple 3526 / Android 1985 / Windows 1131，ACT-01 Activity 聚焦测试已通过。正式 ACT-01 提交 `2491212` 的 GitHub `Apple Build` run `31360092209` 通过共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、工程生成、iPhone/iPad 通用应用构建、macOS 打包与产物上传；第 5 波 Chat 附件又在 GitHub `Apple Build` run `31384177365` 通过 685 项 XCTest（2 跳过）+ 10 项 Swift Testing、通用应用构建和 macOS 打包。删除已下载文件/数据、批量、RSS、文件优先级、BT 协议高级设置、设置写、Activity 主动后台轮询和系统通知继续关闭；真实 NAS 字段、权限、取消/清理行为、Activity NAS 投影、iPad/真机交互继续标记 `PENDING_USER_VALIDATION`。 |
...
| PH8 Windows 对齐 | 基础精选体验与 PHOTO-02 查看器已实现并通过 Windows CI；图片尺寸展示已进入当前源码 | 已完成个人/共享空间、文件夹浏览、公开 List v2 有界时间线、本地搜索/筛选、缩略图、保存副本、PHOTO-03A 单项导入、受限回收站恢复，以及图片/视频右侧预览、前后切换、基础文件元数据、图片尺寸展示、键盘/双击打开和保存副本。基础查看器提交 `4e1272e` 已通过 Windows xUnit、WinUI x64/ARM64 和 Repository Check；更完整 EXIF 白名单、相机/镜头字段、沉浸式显示、高级照片管理及真实 Windows/NAS 验收继续后置 |
...
1. **ACT-01 统一活动中心首片** 已随正式提交 `2491212` 通过 GitHub Apple/Windows/Repository 门禁；接下来只保留真实 NAS、真机与无障碍验收。该首片只做 Download Station 已加载任务快照的 Activity 只读投影，不新增危险写或后台常驻。
2. **CHAT-03 单附件** 的 Apple A0/A1 与 Windows W0/W1 已通过 GitHub Apple、Windows 与 Repository 门禁：下一步以脱敏专用环境验收真实 Chat Server、系统选择器、iPad/Windows 无障碍与弱网边界。Chat 实时继续后置。

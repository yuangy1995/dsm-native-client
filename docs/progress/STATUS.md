...
| iPhone/iPad 客户端 | 第 2 波源码、本地聚焦与云端构建已通过，等待真机验收 | 在前两波 Files/位置/分享/预览、新建/重命名和 PHOTO-01/02 基础上，本波新增 FILE-05 单个普通本地文件同 NAS 复制/移动（无覆盖、提交未知不重放）与 PHOTO-03A PhotosPicker 单项主动导入并复用 Activity。最新六组移动聚焦 45/45 通过；共享 FILE-05 聚焦 103/103 通过；最终提交 `1c7ee4851feb00903327b0599a0d29ea421be8c9` 的 Apple Build 已通过共享 Package、Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包。当前主机无 iPad Simulator，不把云端构建冒充真机运行结论；真机系统选择器和真实 NAS CopyMove/上传副作用仍待验证；目录、批量、跨 NAS、覆盖和回收站写继续关闭。 |
...
| Windows 客户端 | 第 2 波源码与双架构 CI 已通过，等待实机验收 | 在前两波 Application 边界、Files/位置/预览/传输/分享、PHOTO-01、自签证书和 FILE-03 基础上，本波新增 FILE-05 单文件同 NAS CopyMove v3 结果链、普通本地挂载写前门、WinUI 目标选择与跨页面核对 blocker，以及 PHOTO-03A 从 Photos 页选择单项媒体并进入既有 Activity。最终提交 `1c7ee4851feb00903327b0599a0d29ea421be8c9` 的 Windows Build 已通过 815/815 xUnit，WinUI x64/ARM64 均 0 警告、0 错误。真实 NAS、FileOpenPicker、Narrator 与系统生命周期仍待验收。 |
...
- 当前进展：Apple 共享层已新增 `moveToRecycleResult` 与 `restoreFromRecycleResult`，固定 Delete v2 / CopyMove v3，并通过 `DsmFileRepositoryTests` 107/107 与共享 Package 全量；Windows 已新增 `IFileRecycleRepository`、Delete v2 transport、Repository 结果链和 `Files/Recycle` 测试，并已由 GitHub Windows Build 完成编译与完整测试。Files/Photos 上层 UI 入口尚未接入，真实 NAS 回收站策略仍需后置验证。

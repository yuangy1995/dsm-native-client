...
| Synology Chat 模块 | 进行中 | macOS 与 Android 均已接入首次单聊、单附件收发、提醒管理、纯文字定时消息、无附件投票创建和 Socket.IO 实时刷新；Android 另具私人群聊创建、本地置顶、未读汇总、图片与视频预览及有界轮询降级，首次单聊和私人群聊已接入稳定结果、防重复、成员回读与取消后不重放。当前提交 `9067ab3` 已新增 iPhone/iPad 与 Windows 受限纯文字发送：固定 Chat Post v5，成功必须回读确认，提交未知进入核对且不自动重放；Apple 本地聚焦与 GitHub Apple Build run `31334855745` 已通过，Windows 本地轻量门与 GitHub Windows Build run `31334855728` 已通过，Repository Check run `31334855742` 已通过。服务器已读、投票参与/关闭、附件、实时、语音、加密和真实 Chat Server 行为仍待后续切片或实机验收 |
...
| iPhone/iPad 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建、单任务删除、当前活动速率摘要、BT 搜索与 ACT-01 首片均已完成当前源码和云端构建闭环；真机/真实 NAS 待验收 | Files/位置/预览/分享、新建/重命名、同 NAS 复制/移动、回收站、PHOTO-01/02/03A、Chat 只读与受限纯文字发送、Download 常用单任务流程和本地设置已形成移动主流程。BTSearch 固定公开 v1 的目录、启动、列表与清理；能力门、会话内输入、200 字/控制字符门、当前目录归属、搜索中冻结条件但允许关闭、条件变化取消与清旧结果、零提供方恢复态、独立清理、结果创建复用既有单链接安全链均已接入。ACT-01 首片将 Download Station 当前已加载任务快照同步为 Activity 的 NAS 来源，新增暂停态，NAS 项不显示取消或重试。BTSearch 英中资源各 48 项；本机通过共享聚焦 65/65、共享包全量 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone 模拟器 11/11；当前分支本地化为 Apple 3462 / Android 1985 / Windows 1074，ACT-01 Activity 聚焦测试已通过。ACT-01 GitHub `Apple Build` run `31358741106` 通过共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、工程生成、iPhone/iPad 通用应用构建、macOS 打包与产物上传。删除已下载文件/数据、批量、RSS、文件优先级、BT 协议高级设置、设置写、Activity 主动后台轮询和系统通知继续关闭；真实 NAS 字段、权限、取消/清理行为、Activity NAS 投影、iPad/真机交互继续标记 `PENDING_USER_VALIDATION`。 |
...
- 本切片把 Android 已验证的 BTSearch v1 语义前移到 Apple 共享层、iPhone/iPad 与 Windows；两端完整用户入口、状态机、结果创建链、英中资源和专项测试均已落盘，候选提交 `53360d2` 已通过四组云端门禁。
...
- Windows 已完成 `DownloadBtSearch*` 领域模型、兼容默认 Repository seam、公开 v1 `start → list → clean` 仓储、ViewModel 状态机、原生 WinUI `ContentDialog`、`Ctrl+B`、能力门、七类排序及方向、取消/关闭、零提供方、五态/结果态和单结果创建。搜索条件仅驻会话内存，搜索中冻结输入但保留取消/关闭；条件实际变化会取消 token、推进 generation、清空结果/选择并拒绝 A→B→A 迟到结果；保留类别 `_allcat_` 只在展示层去重。Windows BTSearch 英中资源各 61 项，专项测试含 8 项 Repository、15 项 ViewModel 行为和 3 项 source-contract，共 26 项。Windows Build run `31354549859` 已通过 886/886 项 .NET 10 xUnit，WinUI x64 与 ARM64 均 0 警告、0 错误。
- BTSearch 候选门禁：本地化 Apple 3461 / Android 1985 / Windows 1071；请求契约 94 份与 1 份写结果示例；目标 Swift 语法、Apple strings、WinUI XAML、双语 resw、源码安全扫描和 `git diff --check` 均通过。候选提交 `53360d2` 的 Repository Check run `31354549826`、Apple Build run `31354549813`、Android Build run `31354549827` 与 Windows Build run `31354549859` 全部通过；Apple 云端完成共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包，Android 云端完成 JVM、Debug、Release/R8、仪器测试 APK 与 Debug lint。待完成的仅为真实 NAS BTSearch 目录/结果/清理行为，以及 iPad、Windows、Narrator、键盘和高对比实机验收。RSS、文件优先级、BT 协议高级设置、监听目录、NZB 服务器和 Download Station 设置写不属于本切片。
...
- 本地轻量门：iPhone 17 Pro 模拟器 `MobileActivityPresentationTests` 通过；本地化检查通过，资源统计为 Apple 3462、Android 1985、Windows 1074；`TransferActivityPage.xaml` 和 Windows 双语 resw XML 可解析；`git diff --check` 通过。云端门禁：GitHub `Repository Check` run `31358741044` 通过；`Windows Build` run `31358741100` 通过 889/889 项 .NET xUnit，并完成 WinUI x64 与 ARM64 构建；`Apple Build` run `31358741106` 通过共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、工程生成、iPhone/iPad 通用应用构建、macOS 打包与产物上传。
...
- Download Station BT 搜索候选提交 `53360d2` 已完成 Apple shared/mobile 与 Windows Domain/Infrastructure/ViewModel/WinUI 闭环：Apple 共享聚焦 65/65、共享包全量 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone 17 Pro iOS 26.5 模拟器 `MobileDownloadsSafetyTests` 11/11 均通过；Windows 专项自动化为 Repository 8 项、ViewModel 15 项和 source-contract 3 项，共 26 项。
- 同一候选已通过 Repository Check run `31354549826`、Apple Build run `31354549813`、Android Build run `31354549827` 与 Windows Build run `31354549859`。Windows 云端为 886/886 项 xUnit，WinUI x64 与 ARM64 均 0 警告、0 错误；Apple 云端为共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包。BTSearch 候选共同轻量门禁为本地化 Apple 3,461、Android 1,985、Windows 1,071，94 份请求 Fixture 与 1 份写结果示例。上述证据不等同 iPhone/iPad 真机、Windows 真实设备、Narrator 或真实 NAS 搜索与清理行为验收。
- ACT-01 统一活动中心首片已在当前分支完成源码、轻量门和云端门禁：Apple Activity 聚焦测试通过；GitHub `Repository Check` run `31358741044`、`Windows Build` run `31358741100` 和 `Apple Build` run `31358741106` 均通过，其中 Windows 为 889/889 项 xUnit、WinUI x64/ARM64 构建通过，Apple 为共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包。真机、Narrator/键盘/高对比和真实 NAS 任务字段验收仍待完成。
...
- Windows Domain、Infrastructure、Application 和 WinUI 已在 BTSearch 候选提交 `53360d2` 的 Windows Build run `31354549859` 通过 886/886 项 xUnit，WinUI x64 与 ARM64 Release 均 0 警告、0 错误；真实设备、Explorer、Narrator 与系统交互仍待后续验收。
- 本地化检查覆盖 Apple、Android、Windows 的双语键、格式参数、资源引用、英语资源残留中文和生产界面硬编码；当前源码统计为 Apple 3,461 个、Android 1,985 个、Windows 1,071 个双语资源项。
- BTSearch 候选提交 `53360d2` 的 GitHub Apple Build run `31354549813` 已通过共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包；本地另通过共享 Download Station 聚焦 65/65 与 iPhone 17 Pro iOS 26.5 模拟器 `MobileDownloadsSafetyTests` 11/11。Repository Check run `31354549826` 与 Android Build run `31354549827` 同样通过。当前没有 iPad Simulator 或真机结论；桌面云盘配置、事务、主 App 与 Extension 系统边界仍由既有共享包与 macOS 构建持续覆盖，既有临时签名 Release 打包启动结果保持有效。
...
1. 为当前 **ACT-01 统一活动中心首片** 触发 GitHub Apple/Windows/Repository 门禁；全绿后整理为单条简体中文提交。该首片只做 Download Station 已加载任务快照的 Activity 只读投影，不新增危险写或后台常驻。
2. 进入 **CHAT-03 单附件**：先补 Apple typed 单附件发送结果，以及 Windows 上传、缩略图和下载 typed 契约，再接移动端/WinUI 的选择、进度、预览和保存；Chat 实时继续后置。

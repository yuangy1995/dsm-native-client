...
| PH8 Windows 对齐 | 基础精选体验已实现，PHOTO-02 查看器当前分支待 CI | 已完成个人/共享空间、文件夹浏览、公开 List v2 有界时间线、本地搜索/筛选、缩略图、保存副本、PHOTO-03A 单项导入和受限回收站恢复；当前功能分支补入图片/视频右侧预览、前后切换、基础文件元数据、键盘/双击打开和保存副本，已通过本地轻量门，等待 Windows xUnit/WinUI 云端构建和真实 Windows/NAS 验收。更完整 EXIF 白名单、沉浸式显示和高级照片管理继续后置 |

...
| iPhone/iPad 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建、单任务删除、当前活动速率摘要与 BT 搜索已完成当前源码和云端构建闭环；iPad 真机与真实 NAS 待验收 | Files/位置/预览/分享、新建/重命名、同 NAS 复制/移动、回收站、PHOTO-01/02/03A、Chat 只读与受限纯文字发送、Download 常用单任务流程和本地设置已形成移动主流程。BTSearch 固定公开 v1 的目录、启动、列表与清理；能力门、会话内输入、200 字/控制字符门、当前目录归属、搜索中冻结条件但允许关闭、条件变化取消与清旧结果、零提供方恢复态、独立清理、结果创建复用既有单链接安全链均已接入。BTSearch 英中资源各 48 项；本机通过共享聚焦 65/65、共享包全量 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone 模拟器 11/11、本地化 3461 / 1985 / 1071、94 份请求 Fixture 与差异格式门；候选提交 `53360d2` 的 Apple Build run `31354549813` 又通过共享包 675 项 XCTest（2 跳过）+ 10 项 Swift Testing、iPhone/iPad 通用应用构建和 macOS 打包。删除已下载文件/数据、批量、RSS、文件优先级、BT 协议高级设置和设置写继续关闭；真实 NAS 字段、权限、取消/清理行为、iPad/真机交互继续标记 `PENDING_USER_VALIDATION`。 |
...
| Windows 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建、单任务删除、当前活动速率摘要与 BT 搜索已完成当前源码和 Windows 云端构建闭环；真实 Windows/NAS 待验收 | Files/位置/预览/传输/分享、PHOTO-01、证书信任、FILE-03/05/09、PHOTO-03A、Chat 受限纯文字发送和 Download 常用单任务流程已进入 WinUI。BTSearch 固定公开 v1，完成能力门、原生 ContentDialog、`Ctrl+B`、提供方/类别/标题筛选及七类排序与方向、200 字与控制字符门、内存隐私说明、取消/关闭、零提供方、空/筛选空/错误/内容状态、A→B→A 迟到隔离、`_allcat_` 展示去重和单结果创建；创建继续复用既有 typed 单链接链，未知结果同会话不重放。英中资源各 61 项，专项测试为 Repository 8 项、ViewModel 15 项和 source-contract 3 项，共 26 项；本机本地化、XML、契约与差异门通过。候选提交 `53360d2` 的 Windows Build run `31354549859` 通过 886/886 项 .NET 10 xUnit，WinUI x64 与 ARM64 均 0 警告、0 错误。删除已下载文件/数据、批量、RSS、文件优先级、BT 协议高级设置和设置写继续关闭；真实 NAS、Narrator、键盘、高对比和窗口缩放继续 `PENDING_USER_VALIDATION`。 |
...
1. 进入 **ACT-01 统一活动中心**：把 App 前台传输与 NAS/Download 任务按来源分区展示；只做现有任务的只读投影，不新增危险写或后台常驻。
...
4. 使用脱敏专用环境验收 BTSearch 的真实 NAS 搜索目录、结果字段与临时任务清理，以及 iPad/Windows/Narrator/键盘交互；补充当前测试 NAS 的 File Station 与 Download Station 版本，并将实机结果写入[DSM 兼容矩阵](../compatibility/DSM_COMPATIBILITY_MATRIX.md)。

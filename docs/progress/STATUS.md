...
| iPhone/iPad 客户端 | 第 2 波源码与本地聚焦验证完成，等待云端全量与真机验收 | 在前两波 Files/位置/分享/预览、新建/重命名和 PHOTO-01/02 基础上，本波新增 FILE-05 单个普通本地文件同 NAS 复制/移动（无覆盖、提交未知不重放）与 PHOTO-03A PhotosPicker 单项主动导入并复用 Activity。最新六组移动聚焦 45/45 通过；共享 FILE-05 聚焦 103/103 通过。当前主机无 iPad Simulator，iPad 云端构建、真机系统选择器和真实 NAS CopyMove/上传副作用仍待验证；目录、批量、跨 NAS、覆盖和回收站写继续关闭。 |
...
| Windows 客户端 | 第 2 波候选源码与双架构 CI 已通过，等待最终 SHA 与实机验收 | 在前两波 Application 边界、Files/位置/预览/传输/分享、PHOTO-01、自签证书和 FILE-03 基础上，本波新增 FILE-05 单文件同 NAS CopyMove v3 结果链、普通本地挂载写前门、WinUI 目标选择与跨页面核对 blocker，以及 PHOTO-03A 从 Photos 页选择单项媒体并进入既有 Activity。GitHub 候选 run 已通过 815/815 xUnit，WinUI x64/ARM64 均 0 警告、0 错误；整理后的单一提交仍须精确 SHA 复验。真实 NAS、FileOpenPicker、Narrator 与系统生命周期仍待验收。 |

### 第 2 波当前候选（单一简体中文功能提交已全绿，等待合并）

- 基线：`641852b408ae24f8819e4a49cd70df4c8d9e5011`；功能对齐账本见[第 2 波账本](../development/CROSS_PLATFORM_PARITY_WAVE_2_LEDGER_ZH.md)。
- Apple：本机六组移动聚焦 45/45、共享 FILE-05 聚焦 103/103；整理后的提交 `d5e5bec1be170e53a9c1d41b184e047c1183e706` 对应 GitHub `Apple Build` run `31312657926` 通过共享 Package 655 项 XCTest（2 项按环境跳过）、Swift Testing 10 项、iPhone/iPad 通用应用构建和 macOS 打包。
- Windows：同一提交的 GitHub `Windows Build` run `31312657931` 通过 815/815 xUnit，WinUI x64 与 ARM64 均 0 警告、0 错误。
- Android：本波未修改 Android 业务源码；同一提交的 GitHub `Android Build` run `31312657918` 完成完整构建与静态门禁，用于证明新增公共请求 Fixture 未破坏既有目标。
- 仓库：同一提交的 `Repository Check` run `31312657970` 通过；本地请求契约为 92 份 Fixture 与 1 份写结果示例，引用 Fixture 为 3 组/19 项，双语资源为 Apple 3297、Android 1985、Windows 893。
- 临时修正历史已整理为一条简体中文功能提交。合并前仍需确认 `main` 没有未知新提交，并在合并后删除本地与远端候选分支。

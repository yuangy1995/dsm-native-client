...
| iPhone/iPad 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建与单任务删除 UI 已接入并通过云端门禁；真实 NAS 与真机待验收 | 在前两波 Files/位置/分享/预览、新建/重命名和 PHOTO-01/02 基础上，第 2 波新增 FILE-05 单个普通本地文件同 NAS 复制/移动与 PHOTO-03A PhotosPicker 单项主动导入；第 3 波 FILE-09 已在 Files 中接入单个普通本地文件“移到回收站”和回收站位置“恢复”，并在 Photos 网格、时间线和查看器中只对 `#recycle` 普通文件复用同一恢复流程；使用共享 `moveToRecycleResult` / `restoreFromRecycleResult`，提交未知跨页面 blocker 防重放。第 4 波 `codex/wave4-download-control` 已新增 Download Station 单任务暂停/继续结果契约，固定官方 `SYNO.DownloadStation.Task` v1，移动端详情页只显示当前任务可用的暂停或继续操作，提交未知要求刷新核对且不自动重放；随后接入 URL/磁力链接创建入口，复用共享 typed 结果契约和官方 v1 `create`，通过原生 Sheet 输入单个链接，成功时插入确认任务，提交后取消、断线、缺失 task id 或回读不一致均要求核对并防止自动重放；当前切片新增 `.torrent`、`.nzb` 与 `.txt` 任务文件创建，使用官方 `SYNO.DownloadStation.Task` v1 multipart `create`，移动端通过系统文件导入选择单个任务文件，成功只在稳定 task id 回读确认后插入任务，提交未知进入核对且不自动重放；单任务删除仍固定官方 v1 `delete`，始终传 `removeData=false` 等价语义，不删除已下载文件，成功只移除任务，提交未知保留任务并要求在 NAS 中核对；删除已下载文件/数据、批量、RSS/BT 高级和设置写继续关闭。本机已通过共享 `DsmServiceManagementRepositoryTests` 60/60、请求契约校验 92 份 Fixture 与当前 `MobileDownloadsSafetyTests` 8/8 聚焦测试；任务文件创建对应提交 `f2bab7f` 已通过 GitHub Apple Build run `31342949337` 与 Repository Check run `31342949352`。当前主机无 iPad Simulator，不把 iPhone 模拟器结果冒充 iPad 或真机结论。真实 NAS 的任务状态、任务文件兼容、权限、取消时机、删除副作用、iPad 交互和真机仍待验证。 |
...
| Windows 客户端 | Download 单任务控制、URL/磁力创建、任务文件创建与单任务删除 UI 已接入并通过 GitHub Windows 门禁；真实 Windows/NAS 待验收 | 在前两波 Application 边界、Files/位置/预览/传输/分享、PHOTO-01、自签证书和 FILE-03 基础上，第 2 波新增 FILE-05 单文件同 NAS CopyMove v3 与 PHOTO-03A 单项媒体导入；第 3 波 FILE-09 已在 Files 页面接入单个普通本地文件“移到回收站”和回收站位置“恢复”，并在 Photos 文件夹和时间线中只对 `#recycle` 普通文件复用同一恢复流程；复用 `IFileRecycleRepository`、WinUI ContentDialog、普通来源/回收站来源双门与进程会话 blocker。第 4 波 `codex/wave4-download-control` 已在 Windows Domain/Infrastructure/ViewModel/WinUI 页面接入 Download Station 单任务暂停/继续，固定官方 `SYNO.DownloadStation.Task` v1，使用稳定任务基线、单次提交、独立回读和进程会话 review blocker；随后在 WinUI Download Station 页面接入 URL/磁力链接创建入口，复用 typed 结果契约和官方 v1 `create`，通过 ContentDialog 输入单个链接，成功时刷新并呈现确认任务，提交后取消或缺少 task id 进入核对提示并防重放；单任务删除使用固定官方 v1 `delete` 与 `force_complete=false`，确认后只删除任务、不删除已下载文件，成功从列表移除任务，提交未知进入核对提示并防止二次提交；当前切片新增 `.torrent`、`.nzb` 与 `.txt` 任务文件创建，Domain/Infrastructure/ViewModel/WinUI 均使用官方 v1 multipart `create`，WinUI 页面通过当前窗口绑定的系统文件选择器导入单个任务文件，成功按稳定 task id 回读确认并刷新任务列表，提交后取消或返回不可信会进入核对提示并阻止二次上传；删除已下载文件/数据、批量、RSS/BT 高级和设置写继续关闭。本机已通过请求契约校验、差异格式、XAML/resw XML、本地化和源码安全扫描；新增 xUnit 行为/transport 测试已写入但本机无 dotnet/Windows SDK；任务文件创建对应提交 `f2bab7f` 已通过 GitHub Windows Build run `31342949331` 与 Repository Check run `31342949352`，其中 Windows 云端完成 860/860 xUnit 与 WinUI x64/ARM64 构建。真实 NAS、Narrator、键盘、系统生命周期、系统文件选择器和任务删除/创建副作用仍待验收。 |

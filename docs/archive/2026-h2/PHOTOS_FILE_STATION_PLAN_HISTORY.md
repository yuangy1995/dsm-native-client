...
> 旧 File Station 照片方案与跨端阶段记录，已由 [当前照片计划](../../development/NATIVE_DSM_PHOTOS_DEVELOPMENT_PLAN_ZH.md) 取代。保留用于迁移核对，不是当前实现要求；下文的状态、范围与测试数字均属历史。

# 岚仓照片管理开发计划

> 2026-09-09 范围变更：用户已授权 [Synology Photos 完整替换](../../development/NATIVE_DSM_PHOTOS_DEVELOPMENT_PLAN_ZH.md)，该账本优先于本文件原有“两级能力与 File Station 降级”方案。旧方案作为实施历史保留；新模块完全接入 Photos，替换完成后清理旧照片组件。当前仍在施工，不宣称已完成替换。
...
- [当前文件客户端开发与验收计划](../../development/NATIVE_DSM_FILE_APP_DEVELOPMENT_PLAN_ZH.md)

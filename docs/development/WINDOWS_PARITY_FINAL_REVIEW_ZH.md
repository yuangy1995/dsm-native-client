...
后续用户已授权正式提交、推送，以及修复 Android 下载创建兼容问题；本报告保留前一
验证阶段的包和清理证据，正式提交门禁与交付方式见[提交验收记录](PARITY_SUBMISSION_VALIDATION_ZH.md)。
...
此前 Android 35561277878 和 35601196971 中，下载目标目录创建调用仍用 v1，与已纠正
为官方 v2 的 fixture 不一致，有 1 项测试失败。用户随后单独授权修复，Android
35605280419 已确认 1424 项通过、0 失败/跳过，完整构建与 lint 通过；详见提交验收记录。
此前 Windows 分支自动触发的 Apple 35601196965 最终成功，不再是运行中状态；原 Mac
测试包仍对应 Apple Build 35564520773，不将后续 CI 包与已有交付包混淆。

...
## 1.0.8 开关启用闪退修正（2026-09-11，准备发布 1.0.9）

用户报告开启编辑或删除后 App 闪退，重启后开关已开启。所提供报告的崩溃线程为
`FPM-SignalUpdateQueue`，调用链指向 `DesktopDriveWritebackSettingsSheet.signalChanges()` 的回调及
Swift 执行器隔离检查；不是本地权限保存失败。只记录上述必要符号，不将原始报告、设备标识或本机路径写入仓库。

修正复用已有 `DesktopDriveFileProviderCallbackBridge`：页面通过主线程 Task 等待异步结果，
非隔离方法接收可发送的映射值，并在内部创建系统对象；根目录、工作集和错误恢复的完成回调均明确可跨队列。
不关闭 Swift 隔离检查，不增加不安全类型包装；刷新失败显示已保存设置及重新打开挂载的恢复提示，不撤销设置。
权限确认、默认关闭、保存格式、应用身份、签名和 NAS 写入行为均不改变。

新增回归从主线程发起三个后台回调，并验证后台错误回到主线程处理且两项已保存权限保持有效。
聚焦、全量、构建与界面结果见 [本轮验证记录](../releases/MACOS_GITHUB_RELEASE_ZH.md#更新说明与挂载刷新修正2026-09-11)。

`PENDING_USER_VALIDATION`：包含修正的正式签名版本，在用户报告的系统环境中，确认无未处理文件操作后，
分别开启／关闭编辑与删除，确认弹窗结束后 App 保持运行、Finder 刷新、重启后设置一致。
本轮没有操作真实挂载或 NAS 文件，没有将合成回调测试表述为真实系统注册验证。

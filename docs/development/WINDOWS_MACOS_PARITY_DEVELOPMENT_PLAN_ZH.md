...
FILE-04 桌面拖放单文件上传增量已完成源码、本机与云端构建闭环：Windows Files 的普通本地可写目录接受一个具有真实本机路径的 `StorageFile`，拖入有效文件时显示使用系统主题资源的原生投放框；文件夹、多项目、无本机路径、只读位置、加载中、位置切换后的迟到结果和并行选择器操作均零上传并提供双语恢复提示。该入口只把冻结的 profile、目标目录和源路径交给既有 `WindowsTransferPickerService`，继续复用唯一只读源流、`overwrite=false`、ForegroundTransfer、一次 `UploadFileAsync`、typed `MutationResult` 与 Activity；同 profile 下相同目标文件在途时于发送前拒绝并关闭第二个源流，不同目标仍可并行，提交未知仍只提示刷新核对而不重放。该切片不新增 NAS 请求、批量或文件夹上传、覆盖、后台恢复、跨 NAS 或拖放移动。本机 .NET 10 已通过 Files/Picker 聚焦 39/39、Release 全量 xUnit 1009/1009、本地化门禁、XAML/RESW XML 和差异检查；首次完整运行的无关 Preview 元数据时序测试单项与整套原样重跑通过，未修改 Preview 源码。macOS App 构建按预期停在不可执行的 WinUI `XamlCompiler.exe`，不写成本机通过。GitHub Windows Build run `31474456880` 已通过 1009/1009 项 xUnit 与 WinUI x64/ARM64 构建，Repository Check run `31474456939` 已通过。真实 Explorer 拖放、在线占位文件、重复投放、Narrator、高对比、100/150/200% 缩放、窄窗口、触控和真实 NAS 为 `PENDING_USER_VALIDATION`。
...
- 已接入 Photos 文件夹/时间线媒体打开、右侧预览、前后切换、基础文件元数据、图片/基础视频尺寸、拍摄时间、相机品牌/型号、基础视频时长和保存副本；基础查看器提交 `4e1272e` 已通过 Windows xUnit、WinUI x64/ARM64 和 Repository Check。本地媒体元数据白名单切片不新增 NAS 请求，不读取位置、厂商私有、设备序列或镜头序列类隐私元数据；GitHub Windows Build run `31410536634` 已通过 943/943 项 xUnit 与 WinUI x64/ARM64 构建，Repository Check run `31410536680` 已通过。当前分支继续补入页面内全窗口沉浸式查看：F11 切换普通/沉浸状态、左右方向键切换上一项/下一项、Esc 在沉浸式下优先退出、关闭后焦点恢复到照片网格或时间线、Ctrl+S 保存当前查看项，并为查看区域、位置和按钮补双语自动化名称；该切片不新增 NAS 请求、不使用系统级 `AppWindow` 全屏 presenter，也不开放编辑、收藏、删除、相册、人物、地点或标签。GitHub Windows Build run `31414591711` 已通过 949/949 项 xUnit 与 WinUI x64/ARM64 0 警告、0 错误构建，Repository Check run `31414591262` 已通过；真实设备、格式矩阵、Narrator、高对比、窗口缩放和真实 NAS 仍待验收。
...
3. **NAS-01/NAS-02/NAS-04 有界只读详情已扩展为七区**：专用 `NasDetailsPage` 与 typed Repository 现提供系统概览、存储健康、系统更新、套件、计划任务、日志和当前连接。系统区固定使用已登记内部只读 `SYNO.Core.System` v3 `info`，存储区固定使用 `SYNO.Storage.CGI.Storage` v1 `load_info`；更新区复用系统区当前版本，并固定使用 `SYNO.Core.Upgrade.Server.check` v3 及 `user_reading=true`、`need_auto_smallupdate=true`、`need_promotion=false`，任何其他 API、版本或参数组合均在发送前拒绝。系统、存储与更新在 Infrastructure 边界立即窄化为隐私白名单，主机名、原始设备/卷/池标识、路径、序列号、位置、下载地址、任务标识、厂商/硬盘型号、固件和原始响应不进入领域结果或 UI。七区独立表达失败、不可用、空内容和正常状态；更新候选缺失或同版本时不宣告更新，当前版本缺失但明确候选仍可展示，失败不自动重试且不误报最新。本机已通过聚焦 63/63、Release xUnit 1025/1025、本地化、请求/Fixture 契约与 XML 解析；功能分支 GitHub Windows Build run `31481150146` 已通过 1025/1025 项 xUnit 与 WinUI x64/ARM64 0 警告、0 错误构建，Repository Check run `31481150137` 已通过。真实 NAS、Narrator、键盘、高对比和 200% 缩放继续后置验收；更新下载/安装、性能轮询、SMART 操作、连接断开、套件生命周期、任务执行和设置写仍关闭。

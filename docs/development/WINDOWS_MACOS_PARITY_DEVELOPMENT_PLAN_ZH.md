...
- ZIP 创建主流程已接当前文件夹 1～20 项：固定 Compress v3、标准压缩、无密码、不覆盖、一次提交、取消后只回读和非空目标最终核对。ZIP/7z 解压主流程也已接单个当前文件夹归档：固定 Extract v2、无密码、不覆盖、预读顶层输出、一次提交，结果未知跨 Repository 只回读且不重放，并只按顶层路径与类型核对；密码、编码选择、创建 7z、覆盖、递归内容校验和后台恢复继续拆为后续切片。
...
- PHOTO-03 有界批量普通媒体保存副本已完成源码、本机自动化与云端构建：Photos 文件夹网格和主动时间线把保存接入既有原生多选会话，固定 1～20 项；提交前重新核对当前 profile、照片空间、媒体类型、非负大小、完整选择版本和当前选择。所选媒体映射为 `FileDownloadBatchItem`，严格复用 `BoundedFileDownloadBatch` 与 `WindowsTransferPickerService`，一次选择本地文件夹并由 Activity 跟踪，不新增 NAS 请求或 Photos 专用传输实现。批次内大小写不敏感同名、Windows 非法文件名或本地已有同名目标会整批零下载拒绝；全部目标先预留且不覆盖，下载严格串行，单项失败继续，取消停止后续并保留已经保存的副本。本机聚焦 xUnit 50/50、Release 全量 xUnit 1196/1196、本地化、XAML/RESW XML 与差异检查已通过；macOS WinUI App 构建停在不可执行的 Windows `XamlCompiler.exe`，不记为通过。Windows Build run `31546988775` 已通过 1196/1196 与 WinUI x64/ARM64 构建，Repository Check run `31546988693` 已通过。真实 Windows/NAS、Narrator、高对比、200% 缩放、窄窗口、键鼠/触控、系统文件夹选择器和取消时机为 `PENDING_USER_VALIDATION`。
- FILE-09 / PHOTO-03 有界批量恢复已完成源码、本机自动化与云端构建：Files 当前回收站文件夹、Photos 文件夹网格和主动时间线均可选择 1～20 个规范 `#recycle` 项目，一次确认后严格串行复用既有 `RestoreFromRecycleAsync` typed 结果链。完整来源版本、当前 profile、位置或照片空间、选择及可解析原目的地会在提交前重核；恢复固定不覆盖，大小写不敏感的重复目的地整批零提交。明确失败继续，提交未知、异常或提交后取消停止余项、写入复核门且不重放；成功后只刷新仍匹配的来源。本机聚焦 63/63、Release xUnit 1199/1199、本地化、23 个 XAML/RESW XML 与差异检查已通过；macOS WinUI App 构建完成依赖层后停在不可执行的 Windows `XamlCompiler.exe`，不记为通过。Windows Build run `31548672121` 已通过 1199/1199 与 WinUI x64/ARM64 0 警告、0 错误，Repository Check run `31548672132` 已通过；永久删除、清空回收站、覆盖/改名恢复、跨 NAS、并行、自动重试和后台恢复继续关闭，真实 NAS/Windows 与辅助功能为 `PENDING_USER_VALIDATION`。
...
本机关闭 PRI 生成后聚焦 29/29、Release 全量 xUnit 1315/1315、本地化、请求/Fixture 契约、XAML/RESW XML 与差异检查通过；功能分支 Repository Check run `31585274223` 与 Windows Build run `31585274234` 已通过，主分支 Repository Check run `31585568876` 与 Windows Build run `31585568768` 也已通过 1434/1434 项 xUnit 和 WinUI x64/ARM64 构建。MD5、递归本地回退、批量统计、后台持久化和 VFS 不在本切片；真实 NAS/Windows 与辅助功能为 `PENDING_USER_VALIDATION`。

### FILE-06/08 异步递归搜索与安全文本编辑增量

Windows Files 异步递归搜索已完成源码主流程：固定 `start/poll/list/stop`，最多呈现 2,000 项；搜索期间显示进度，读取失败提供明确错误和重试，达到上限时提示结果已截断。离开或取消搜索会收敛任务，不把失败伪装成空结果。

白名单文本编辑仅对 UTF 文本主流程开放，编码后大小不得超过 5 MiB，并且必须先取得服务端强内容版本。编辑时保留原始字节长度、强版本和 SHA；保存前独立重读并核验原内容，用户通过原生确认后只上传一次，提交后不复用页面取消令牌，独立下载目标内容并核对 SHA。提交状态不明、提交后取消或回读失败时不自动重放，并阻止同一草稿再次保存；无法取得强版本时编辑入口不可用。格式整理仍只作用于本地草稿，不改变上述提交边界。
...
Windows Photos 已在文件夹、主动时间线和查看器三个入口补入当前普通媒体的分享链接管理。用户可打开原生 ContentDialog，只看到路径与当前媒体完整路径精确相等的既有分享链接，并可复制链接或二次确认撤销单条链接。

该切片复用 FILE-07 的公开 File Station Sharing v3 `list`/`delete` 链和共享管理 ViewModel，不新增 Synology Photos 私有请求或 NAS 请求类型。列表过滤只接受当前照片空间根下的普通图片/视频、非回收站、已知大小和修改时间；不按文件名前缀、目录、旧缩略图或 Synology Photos 字段推断归属。撤销前继续冻结稳定 ID、路径、URL、密码状态和到期日，提交前由 Repository 重读完整列表核对，提交后只有稳定 ID 消失才确认成功；未知、取消或断线只回读并要求用户核对，不自动重放。密码不进入界面、剪贴板或日志。

本地已通过 Release xUnit 1497/1497、Repository Check 可跑脚本、本地化、XAML/RESW XML 与差异检查；功能分支 Windows Build run `31684348377` 已通过 1497/1497 项 xUnit 与 WinUI x64/ARM64 构建，Repository Check run `31684348386` 已通过。批量共享、批量撤销、编辑密码/到期日、系统分享和 Synology Photos 私有分享继续关闭。真实 NAS 撤销传播、权限差异、真实 Windows/Narrator、高对比、200% 缩放、窄窗口、键盘、鼠标和触控为 `PENDING_USER_VALIDATION`。

...
3. **NAS-01/NAS-02/NAS-04 有界只读详情已扩展为八区**：专用 `NasDetailsPage` 与 typed Repository 现提供系统概览、存储健康、系统更新、当前账号共享访问、套件、计划任务、日志和当前连接。共享访问复用公开 `SYNO.FileStation.List.list_share` v2，以 200 项分页、500 项源上限读取，并在 Infrastructure 内按路径去重后丢弃路径；只保留本地生成 ID、共享名称、读写/只读/未知等级和明确删除能力，远程挂载、`#recycle`、所有者、账号、路径、原始权限对象及其他附加字段不进入领域结果或 UI。缺少权限位保持未知，不把不可见条目解释为拒绝；说明与专属空态明确这不是管理员 ACL 矩阵。八区独立表达失败、不可用、空内容、截断和正常状态，所有共享写、配额和管理员权限编辑继续关闭。本机已通过聚焦 67/67、Release xUnit 1029/1029、本地化与 XML 解析；功能分支 GitHub Windows Build run `31482751695` 已通过 1029/1029 项 xUnit 与 WinUI x64/ARM64 0 警告、0 错误构建，Repository Check run `31482751677` 已通过。真实 NAS 的普通账号、权限缺失、分页和远程挂载，以及 Narrator、键盘、高对比和 200% 缩放继续后置验收；更新下载/安装、性能轮询、SMART 操作、连接断开、套件生命周期、任务执行和设置写仍关闭。

...
> 更新：2026-09-15。本文合并旧照片计划与替换账本，作为当前照片范围、实现位置和剩余工作的唯一入口。[旧 File Station 方案](../archive/2026-h2/PHOTOS_FILE_STATION_PLAN_HISTORY.md)仅用于迁移追溯，不再作为新实现要求。
...
本波次由用户明确授权 Android、iPhone、iPad、Windows；基线为 `2243e731`，
`apple/Apps/DsmMac/**` 只读。以下是实施前范围，验证结果在文末更新，不把目标当作完成证据。

| 切片 | macOS 证据 | 目标端等价结果与交互 | 契约／安全级别 | 验证／非目标 |
| --- | --- | --- | --- | --- |
| 身份、读取与日期定位 | `SynologyPhotosRepository.swift`、`SynologyPhotosModel.swift` | 四端均以账号＋个人空间＋项目 ID 分页；移动端年月选择器，Windows 键鼠导航；禁止旧扫描降级 | 已记录 Photos 读取契约；只读、凭据隔离 | 待实现及聚焦回归；共享空间关闭 |
| 相册、分类、搜索与筛选 | 同上及 `PhotoFilterPanel` | 四端全库搜索、12 类条件、文件夹、相册、分类和共享读取；移动端表单，Windows 原生控件 | 既有私有读取；能力失败独立展示 | 待实现；不新增分享、相册或上传写入 |
| 媒体、保存与单项删除 | `SynologyPhotoPreview`、`prepareDeletion/deletePhoto/reviewDeletion` | 大图、实际视频源、实况视频单元、属性、系统另存为；删除确认后重核身份／权限，未知只查询 | 原件读取及个人单项危险写；沿用已授权门禁 | 聚焦自动化＋构建后 `PENDING_USER_VALIDATION`；组合 ZIP、共享／批量删除关闭 |
| 性能与生命周期 | 上述模型、目标端已有媒体传输和缓存 | 有界缩略图、分页去重、取消和会话代次隔离；不全库预载、不把凭据放媒体 URL | 认证／隐私／资源释放 | 目标平台回归；大图库与设备内存后置验证 |

Apple 移动端直接编译不含 AppKit 的既有 `SynologyPhotosModel.swift`，复用共享领域与网络包，
不复制一份会漂移的状态机、不修改 Mac 源文件。文件管理及 Android 已批准备份依赖仍保留；
旧 File Station 照片组件只供既有回归／兼容调用，不得成为新照片工作区的数据源。

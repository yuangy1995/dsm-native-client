...
## 2026-09-15：五端原生照片与玻璃界面对齐账本

本波次获得 Android、iPhone、iPad、Windows 的明确修改授权。此前“本轮不改 Android”及“不复刻 macOS 视觉”的限制不适用于本波次；macOS App 仍为只读基准。Windows 重建原生 WinUI 展示层，保留已验证的认证、证书、领域及系统集成，避免以重写界面为由丢失安全行为或改动存储身份。所有交付通过专用分支和 PR，不直接更新主分支。

| 切片 | macOS 证据 | 目标端等价结果与交互转换 | 契约／风险 | 验证状态 |
| --- | --- | --- | --- | --- |
| 图库读取 | `SynologyPhotosRepository.swift`、`SynologyPhotosModel.swift` | 四端使用账户＋空间＋项目 ID；时间线、目录、相册、搜索、筛选、分类、共享只读；无 File Station 扫描降级 | `photos-library-read.md`；私有只读；精确版本与请求格式；未知空间拒绝 | 实施中，逐提交记录构建及测试 |
| 媒体 | 同上与 `SynologyPhotoPreview` | 缩略图、大图、视频／实况、属性、原件；移动端触控与系统分享，桌面键盘与文件保存 | 凭据仅随同源受信任传输；下载暂存、大小校验、取消清理；不把链接交给外部播放器 | 实施中；真实格式兼容为 `PENDING_USER_VALIDATION` |
| 视觉 | `MacGlassSurface`、`MacAppearancePalette`、`SynologyPhotosView` | 同一层次与圆角／间距；Android 原生绘制，Apple 系统 Liquid Glass，Windows WinUI Acrylic；iPad 宽屏与 iPhone 紧凑布局 | 不改变账户、权限、数据库和最低系统；尊重减少透明度、对比度、动态文字与降低动态效果 | 实施中；实际像素／设备帧率尚未验收 |
| 性能与隔离 | 分页和迟到结果回归、缩略图身份 | 增量分页、有界缩略图缓存与并发；切换账户、页面、筛选后取消并拒绝旧响应；禁止全库原图预取 | 只读；缓存键包含账户、空间、项目及修订 | 实施中，需聚焦回归及大图库设备测试 |
| 移动只读取舍 | Mac 删除／整理等桌面入口 | iPhone 与 iPad 均不开放未经本端验证的远端写；查看共享与请求可用，创建／撤销／权限修改不开放 | Mac 的单张删除证据不等于其他平台危险写通过；保留官方 Photos 替代路径 | 明确边界；不是等待真机后自动解锁 |

系统玻璃采用官方公开 API：iOS 26 起的 `glassEffect` 在 iOS 27 使用系统新材质；不虚构“iOS 27 专用”修饰符。旧 SDK／旧系统使用原生 Material 降级。Windows 仍是原生 WinUI，而非 WebView／Electron；玻璃只施加于导航和控制表面，照片内容不模糊。平台测试和设备证据在本节更新，不能以一个平台构建代替另一个。

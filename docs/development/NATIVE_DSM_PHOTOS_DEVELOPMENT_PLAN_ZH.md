...
Windows PH8/PHOTO-03 现进一步完成有界批量移入回收站源码主流程：文件夹网格和主动时间线均可选择 1～20 个普通图片或视频，时间线允许当前照片空间内跨文件夹选择，但严格拒绝根外、相似前缀、回收站、未知大小和无已发现位置项目。该能力不新增 Photos 或 NAS 请求，复用 FILE-09 的单次提交、权限/身份预检、最终回读和提交未知防重放；确认前还会重核 profile、空间、完整版本与选择。本机聚焦 52/52、Release xUnit 1182/1182、本地化与 XML 门禁通过；Windows Build run `31537462070` 已通过 1182/1182 与 WinUI x64/ARM64 构建，Repository Check run `31537463067` 已通过。这里的“批量”只覆盖移入 NAS 回收站；批量移动/复制/导出/恢复、相册、人物、地点、标签、系统图库删除和后台整库处理仍是独立后续能力。

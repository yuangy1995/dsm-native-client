...
Windows PH8/PHOTO-03 现已具备两条有界批量主流程：文件夹网格和主动时间线均可选择 1～20 个普通图片或视频，时间线只接受当前照片空间根的严格后代并允许跨文件夹选择。批量移入回收站继续复用 FILE-09，已通过 Windows Build run `31537462070` 的 1182/1182 项 xUnit 与 WinUI x64/ARM64 构建及 Repository Check run `31537463067`；批量同 NAS 移动复用 FILE-05，同一无覆盖目标下的跨目录同名会拒绝，严格串行且未知结果停止余项、不重放，本机聚焦 56/56、Release xUnit 1193/1193、本地化与 XML 已通过，Windows Build run `31540041995` 与 Repository Check run `31540042025` 已通过。批量复制/导出/恢复、相册、人物、地点、标签、系统图库删除和后台整库处理仍是独立后续能力。

...
| ACT-01 · B | App 传输与 NAS 后台任务分源、速度/剩余时间、筛选、分页、通知 | 活动中心、Toast/系统通知、托盘摘要 | **核心**：前台传输与 NAS 任务分源状态；Windows 已补可见时立即读取、5 秒单飞轮询、手动刷新、失败保留和离页停止，系统后台与通知仅在后续获批时增强 | `ActivityTask.swift`、`WorkspaceView.swift` |

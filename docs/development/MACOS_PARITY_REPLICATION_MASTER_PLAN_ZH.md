...
| FILE-01 · A | 共享目录、文件夹分页、列表/图标、排序/分组、面包屑、搜索、当前账号可见容量 | 列表/网格切换、BreadcrumbBar、键盘搜索、多选；Windows Files 已从现有公开共享根响应提取总量/已用/剩余/卷数，失败不阻断浏览且不新增请求 | **核心**：层级浏览、分页、排序/筛选、搜索与状态恢复；移动端共享根已显示当前账号可见容量并独立处理失败 | `WorkspaceView.swift`、`DsmFileRepository.swift` |
...
| CHAT-04 · C | 删除本人消息、关闭会话、消息转发、服务端消息置顶/取消置顶；语音发送和完整加密实现不存在 | 可发现的消息/会话菜单与结果回读 | **受限**：iPhone/iPad 已开放本人消息删除；关闭会话、转发、服务端置顶、高级管理、语音和加密当前不做 | `ChatWorkspaceModel.swift`、`DsmChatRepository.swift` |
...
Windows PHOTO-03 已继续补入 1～20 项普通媒体同 NAS 批量移动源码闭环：文件夹与时间线复用同一批量选择会话和 FILE-05 typed 链，时间线只接受当前照片空间根的严格后代并允许混合父目录；所有项目落入同一目标，因此跨目录大小写不敏感同名会零提交拒绝。目标必须是已发现的普通本地可写目录且不能是任一来源父目录，严格串行提交中明确失败继续，未知、异常和提交后取消停止余项并阻断重放；提交前重核 profile、空间、模式、完整版本和选择。本机聚焦 56/56、完整 xUnit 1193/1193、本地化及 XML 通过；macOS WinUI App 构建停在 Windows `XamlCompiler.exe`，不记为本机通过。Windows Build run `31540041995` 已通过 1193/1193 与 WinUI x64/ARM64 构建，Repository Check run `31540042025` 已通过。真实 Windows/NAS 与辅助功能为 `PENDING_USER_VALIDATION`；跨 NAS、覆盖、自动改名、撤销、后台恢复和系统图库移动继续关闭。

Windows PHOTO-03 已继续补入 1～20 项普通媒体同 NAS 批量复制源码闭环：文件夹和时间线把复制、移动与回收收敛到单一原生多选会话；时间线混合父目录仍受照片空间根约束，同一无覆盖目标下的跨目录同名零提交拒绝。复制复用 FILE-05 的同 NAS 可写目标、冲突、防重复、串行一次提交和最终回读，不要求删除权限，也不刷新来源；未知、异常与提交后取消停止余项且不重放。本机聚焦 57/57、完整 xUnit 1194/1194、本地化与 XML 通过；Windows Build run `31541423773` 已通过 1194/1194 与 WinUI x64/ARM64 构建，Repository Check run `31541423774` 已通过。跨 NAS、覆盖、自动改名、撤销、后台恢复及系统图库复制继续关闭，真实 Windows/NAS 与辅助功能为 `PENDING_USER_VALIDATION`。

Windows PHOTO-03 已继续补入 1～20 项普通媒体批量保存副本源码闭环：文件夹网格和主动时间线复用既有原生多选，提交前重核当前 profile、照片空间、媒体类型、非负大小、完整选择版本与当前选择，再映射 `FileDownloadBatchItem`。保存链复用 `BoundedFileDownloadBatch` 和 `WindowsTransferPickerService`，只选择一次本地文件夹，由 Activity 跟踪，不新增 NAS 请求；大小写不敏感同名、Windows 非法名或本地已有同名会整批零下载拒绝，全部目标先预留并保持不覆盖。执行严格串行，单项失败继续，取消停止后续并保留已保存项目。本机聚焦 xUnit 50/50、Release 全量 xUnit 1196/1196、本地化、XML 与差异检查已通过；macOS WinUI App 构建停在不可执行的 Windows `XamlCompiler.exe`，不记为通过。GitHub Windows Build run `31546988775` 已通过 1196/1196 与 WinUI x64/ARM64 构建，Repository Check run `31546988693` 已通过。真实 Windows/NAS、Narrator、高对比、200% 缩放、窄窗口、键鼠/触控、系统文件夹选择器和取消时机为 `PENDING_USER_VALIDATION`。

Windows FILE-09 / PHOTO-03 已继续补入 1～20 项原位置批量恢复源码闭环：Files 当前回收站文件夹、Photos 文件夹网格和主动时间线复用同一原生多选、确认、进度与守恒汇总，并共享既有 FILE-09 Repository，不新增请求。选择只接受规范 `#recycle` 来源并冻结完整版本，提交前重核 profile、位置/照片空间、选择和可解析目的地；恢复固定不覆盖，重复目的地整批拒绝。串行执行中明确失败继续，未知、异常或提交后取消停止余项、进入会话复核门且不重放。本机聚焦 63/63、Release xUnit 1199/1199、本地化、23 个 XAML/RESW XML 与差异检查已通过；macOS WinUI App 构建停在不可执行的 Windows `XamlCompiler.exe`，不记为通过。Windows Build run `31548672121` 已通过 1199/1199 与 WinUI x64/ARM64 0 警告、0 错误，Repository Check run `31548672132` 已通过。永久删除、清空回收站、跨 NAS、覆盖/改名、并行和后台恢复继续关闭；真实 Windows/NAS 与辅助功能为 `PENDING_USER_VALIDATION`。
...
Apple 移动 FILE-05 已继续补入单个普通本地文件夹，以及同一当前目录 1～20 个普通本地文件的同 NAS 复制/移动源码闭环。单项文件夹走既有 SwiftUI 目标 Sheet，排除源目录整棵子树；目录结果只核对路径、名称和类型，不依赖 DSM 非稳定目录大小。批量模式仍只开放普通文件，列表和网格提供原生选择模式、20 项上限及可见复制/移动动作；一次选择普通本地可写目标后严格串行复用 CopyMove v3 typed 链，固定无覆盖。明确失败继续并记录原因；写前取消停止；未知、部分结果、异常或回读不一致停止余项，以稳定路径身份 blocker 禁止重放。完成页显示守恒计数与失败/取消文件，VoiceOver 可读选择状态和上限。本机 iPhone 17 Pro 模拟器 CopyMove 聚焦 23/23、共享 `DsmFileRepositoryTests` 112/112、`RequestFixtureContractTests` 29/29、本地化和差异检查已通过；批量文件此前本机聚焦 30/30、DsmMobile 全量 470/470 已通过。批量目录、跨目录来源、跨 NAS、覆盖、自动改名、拖放、撤销与后台恢复保持关闭；真实设备、真实 NAS、VoiceOver、最大动态文字和 iPad 外接输入为 `PENDING_USER_VALIDATION`。
...
### iPhone/iPad FILE-05 单文件夹复制/移动增量

iPhone/iPad Files 已补入当前单个普通本地文件夹的同 NAS 复制/移动入口。该能力对应 macOS 和 Windows 已有的“把文件夹作为一个项目复制或移动到同 NAS 目标”的用户结果，并继续转换为 SwiftUI 原生目标选择 Sheet；批量目录仍不开放。

实现复用公开 `SYNO.FileStation.CopyMove` v3 与既有 typed 结果链，不新增请求契约。入口要求当前 profile、普通本地位置、可见列表项、完整路径和可写目标；目标浏览会排除源目录整棵子树，提交仍固定 `overwrite=false`、一次提交、提交未知只回读不重放。目录确认只核对路径、名称和类型，不依赖 DSM 非稳定目录大小；文件仍继续核对大小和修改时间。本机 iPhone 17 Pro 模拟器 CopyMove 聚焦 23/23、共享 `DsmFileRepositoryTests` 112/112、`RequestFixtureContractTests` 29/29、本地化和差异检查已通过；真实 iPhone/iPad、真实 NAS 目录副作用、权限变化、同名冲突、弱网取消、VoiceOver、最大动态文字和 iPad 键盘/指针为 `PENDING_USER_VALIDATION`。

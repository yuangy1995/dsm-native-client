...
| FILE-04 · A/B | 系统选择器上传、覆盖确认、文件/文件夹/批量下载、取消，以及有恢复元数据时的继续/重试；上传重启发送，已知大小普通下载才用严格 Range 分片继续 | FileOpenPicker/FolderPicker/FileSavePicker；Files 已接 1～20 个文件及一个最多 20 文件、20 目录、8 层的小文件夹有界前台上传，保持无覆盖、父目录优先和严格串行；后台传输与系统通知仍后置 | **核心**：用户选择的单文件导入、导出、分享、取消；文件夹/大批量与常驻后台当前不做 | `WorkspaceModel.swift`、`DsmFileRepository.swift` |

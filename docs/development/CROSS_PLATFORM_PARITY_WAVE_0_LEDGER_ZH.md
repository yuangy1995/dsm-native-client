...
3. **W3 Photos 与 Chat**：PHOTO-01 时间线、PHOTO-03A 单项导入和 Chat 受限纯文字发送已完成当前低风险闭环；Windows PHOTO-02 查看器/基础元数据已接入文件夹与时间线媒体打开、右侧预览、前后切换和保存副本，并随提交 `4e1272e` 通过 Windows xUnit、WinUI x64/ARM64 和 Repository Check；更完整 EXIF/沉浸式细节、Chat 附件后的实时、更多消息动作、照片更完整管理和真实设备/NAS 验收仍按套件版本与真实环境验证推进。

...
- 2026-09-19 源码纠正：明确的 DSM 105/106/107/119 拒绝显示权限/会话原因，不再永久
  标为待核对；同一操作标识记住该拒绝，不重复发送。117、HTTP/网络/畸形回执和取消
  仍保持未知，只有新的明确用户确认才能启动新预检。新增回归待 Mac 执行，无新环境证据。
...
- 2026-09-19 用户反馈 macOS 1.0.9 删除失败后，源码预检移除对完整媒体详情的依赖：
  仍为原 Browse.Item.get v5 和完整目标身份比较，但不请求/解析 EXIF、地址或转换
  字段；成功空回读也使用最小身份响应模型。权限和防重复未放宽，新合成测试待 Mac
  执行。没有新增真实 NAS 证据，具体失败提示仍待用户补充。

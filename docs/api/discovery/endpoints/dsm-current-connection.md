...
## 2026-09-26 Apple PID 兼容修复

兼容整数与原有文本形式的 `pid`，只对该标识字段执行转换，拒绝小数和越界编号。
其余权限、身份摘要、重复目标、预检与断开回读保持不变。当前连接错误不再误报
为区域与时间错误。来源和现场观察限制见 [本次观察](../environments/2026-09-26-nas-settings-read-observation.md)。

2026-09-27 补充：已在当前浏览器成功响应中核对数字 PID，以及文本身份字段和
布尔 `is_current_connected/can_be_kicked`；未执行断开。见
[实测记录](../environments/2026-09-27-nas-settings-live-validation.md)。

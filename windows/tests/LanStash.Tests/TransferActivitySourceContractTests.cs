...
public sealed class TransferActivitySourceContractTests
{
    ...
    public void ActivityPageShowsScopedStatesAndStopsPollingWhenHidden()
    {
        ...
        Assert.Contains("_downloadRefresher.LoadMoreAsync()", source);
        Assert.Contains("_fileRefresher.LoadMoreAsync()", source);
        Assert.Contains("TransferActivityLoadMoreDownloads", xaml);
        Assert.Contains("TransferActivityLoadMoreFiles", xaml);
        Assert.Contains("ActivityList.ItemsSource = _presentations", source);
        Assert.DoesNotContain("ActivityList.ItemsSource = items", source);
    ...
    public void WindowsNotificationsUseGenericTextAndRouteOnlyToActivity()
    {
        ...
        Assert.Contains("args.Arguments.ToDictionary", service);
        Assert.Contains("AppNotificationManager.Default.Unregister()", service);
        Assert.DoesNotContain("UnregisterAll", service);
        Assert.Contains("if (_registered) return;", service);
        Assert.Contains("if (_clients == 0 && !_keepListening) StopListening();", service);
        Assert.Contains("_notificationBackend.StartListening()", window);
        Assert.Contains("_notificationBackend.Dispose()", window);
        Assert.Contains(".AddArgument(\"route\", \"activity\")", service);
        Assert.Contains(".AddArgument(\"context\", context)", service);
        Assert.Contains("ReferenceEquals(source, _transferNotifications)", window);
        Assert.Contains("generation != Volatile.Read(ref _notificationGeneration)", window);

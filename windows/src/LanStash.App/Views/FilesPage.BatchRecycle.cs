...
public sealed partial class FilesPage
{
    ...
    private async Task ShowBatchRecycleAsync()
    {
        ...
        async Task RenderAsync()
        {
            ...
            dialog.Content = BuildBatchRecycleContent(model, localization);
    ...
    private static StackPanel BuildBatchRecycleContent(
        ...
        LocalizationService localization)
    {
        ...
        if (model.State == FileRecycleBatchState.Confirming)
        {
            panel.Children.Add(new InfoBar
            {
                ...
                Message = localization.Get("FileRecycleBatchConfirmMessage"),
            });
            return panel;
        }
        ...
        var message = new InfoBar
        {
            ...
            Message = model.State == FileRecycleBatchState.Completed
                ? FormatBatchRecycleSummary(localization, model.Summary)
    ...
    private void ShowBatchRecycleSummary(FileRecycleBatchSummary summary)
    {
        FileRecycleBatchStatus.Severity = summary.NeedsReviewCount > 0 ||
            ...
            ? InfoBarSeverity.Warning
            : InfoBarSeverity.Success;
        FileRecycleBatchStatus.Message = FormatBatchRecycleSummary(
            LocalizationService.Current,
            summary);
        FileRecycleBatchStatus.IsOpen = true;
    }

    private static string FormatBatchRecycleSummary(
        LocalizationService localization,

...
public sealed partial class ContainerImageDeleteDialogContent : UserControl, INasSettingsDialogContent
{
    private readonly IContainerManagerRepository _repository;
    private readonly ContainerImageDeletionViewModel _model = new();
    ...
    public string? PrimaryButtonResourceKey => "ContainerImageDeleteAction";
    public bool NeedsParentRefresh => _model.NeedsParentRefresh;
    public ContainerImageDeleteDialogContent(IContainerManagerRepository repository)
    ...
    private void Refresh()
    {
        ...
        PendingNotice.Text = L.Format("ContainerImageDeletePending", string.Join(Environment.NewLine, _model.Pending.SelectMany(item => item.Baselines).Select(item => item.Name).Distinct()));
        ...
        SelectionNotice.Text = L.Format("ContainerImageDeleteSelection", _model.SelectionNames);
